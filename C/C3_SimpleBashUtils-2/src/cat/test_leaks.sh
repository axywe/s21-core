...
testing()
{
    t=$(echo $@ | sed "s/VAR/$var/")
     valgrind --leak-check=full --log-file=mem-leak-amsHelper.log ./s21_cat $t 
    leak=$(grep total mem-leak-amsHelper.log ./s21_cat | awk '{print $5}')
    leak1=$(grep total mem-leak-amsHelper.log | awk '{print $7}')
    (( COUNTER++ ))
    if [[ $leak == $leak1 ]]
    then
      (( SUCCESS++ ))
